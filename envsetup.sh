...
- evremote: Add git remote for Evervolv Gerrit Review
...
function evremote() {
    git remote rm evremote 2> /dev/null
    if [ ! -d .git ]
    then
        echo ".git directory not found. Please run this from the root directory of the Android repository you wish to set up."
    fi
    GERRIT_REMOTE=$(cat .git/config  | grep git://github.com | awk '{ print $NF }' | sed s#git://github.com/Evervolv-ICS/##g)
    if [ -z "$GERRIT_REMOTE" ]
    then
